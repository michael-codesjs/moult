...
const git: SimpleGit = simpleGit({ baseDir, binary, maxConcurrentProcesses, trimmed });
...
(async () => {
    ...
    const modifiedDeployablePackages = deployablePackages.filter(deployablePackage => {
        deployablePackage.location = deployablePackage.location.replace(baseDir, "");
