resource "aws_dynamodb_table" "user_credential_materialized_view_dynamoDb_table" {
  ...
  name = "moult-user-credential-materialized-view-${var.stage}"

  tags = {
    Name        = "moult-user-credentials-materialized-view-${var.stage}"
    Application = "moult"
...
resource "aws_ssm_parameter" "user_credentials_table_name" {
  name  = "/moult/${var.stage}/domain/authentication/service/user-credentials/infrastructure/storage/table/user-credentials/name"
  type  = "SecureString"
...
resource "aws_ssm_parameter" "user_credentials_table_arn" {
  name  = "/moult/${var.stage}/domain/authentication/service/user-credentials/infrastructure/storage/table/user-credentials/arn"
  type  = "SecureString"
...
resource "aws_ssm_parameter" "credentials_table_stream_arn" {
  name  = "/moult/${var.stage}/domain/authentication/service/user-credentials/infrastructure/storage/table/user-credentials/stream/arn"
  type  = "SecureString"
