...
const initializeTerraform = async () => {
  ...
  const stateFileKey =
    'domain/media-storage/service/photo/infrastructure/terraform.tfstate'
...
const deployInfrastructure = async () => {
  const spinner = ora(
    `Deploying photos service infrastructure to ${chalk.black(stage)}.`,
  )
  spinner.start()
  ...
  if (success)
    return (
      spinner.succeed(
        `Successfully deployed photos service infrastructure to ${chalk.black(stage)}.`,
      ) || true
    )
  else
    spinner.fail(
      `Failed to deploy photos service infrastructure to ${chalk.black(stage)}.`,
    )
}

const deployApplication = async () => {
  const spinner = ora(
    `Deploying photos service application to ${chalk.black(stage)}.`,
  )
  spinner.start()
  ...
  if (success)
    return (
      spinner.succeed(
        `Successfully deployed photos service application to ${chalk.black(stage)}.`,
      ) || true
    )
  else
    spinner.fail(
      `Failed to deploy photos service application to ${chalk.black(stage)}.`,
    )
}
