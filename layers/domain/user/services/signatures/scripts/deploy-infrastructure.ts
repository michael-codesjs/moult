...
const deployInfrastructure = async () => {
  const spinner = ora(
    `Deploying signatures service infrastructure to ${chalk.black(stage)}.`,
  )
  spinner.start()
  ...
  if (success)
    return (
      spinner.succeed(
        `Successfully deployed signatures service infrastructure to ${chalk.black(stage)}.`,
      ) || true
    )
  else
    spinner.fail(
      `Failed to deploy signatures service infrastructure to ${chalk.black(stage)}.`,
    )
}

const deployApplication = async () => {
  const spinner = ora(
    `Deploying signatures service application to ${chalk.black(stage)}.`,
  )
  spinner.start()
  ...
  if (success)
    return (
      spinner.succeed(
        `Successfully deployed signatures service application to ${chalk.black(stage)}.`,
      ) || true
    )
  else
    spinner.fail(
      `Failed to deploy signatures service application to ${chalk.black(stage)}.`,
    )
}
