...
export class User extends AggregateRoot {
    ...
    for (const event of events) {
      switch (event.name) {
        ...
        case 'USER_USERNAME_UPDATED':
          user_credential.attributes.set({ username: event.payload.username })
          break
        default:
  ...
  updateUsername(username: string) {
    this.attributes.set({ username })

    // const isUpdateable = this.attributes.isUpdateable()
    // if (!isUpdateable) throw new NotUpdateable() TODO: debug is updateable

    this.registerDomainEvent({
      name: 'USER_USERNAME_UPDATED',
      payload: { username, id: this.attributes.get('id') },
      source: 'moult.domain.user.services.user',
      date: new Date(),
    })
  }
