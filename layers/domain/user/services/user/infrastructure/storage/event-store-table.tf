...
resource "aws_ssm_parameter" "event_store_table_name" {
  name  = "/moult/${var.stage}/domain/authentication/service/user/infrastructure/storage/moult-user-event-store-table/name"
  type  = "SecureString"
  value = aws_dynamodb_table.event_store_table.name
}

resource "aws_ssm_parameter" "event_store_table_arn" {
  name  = "/moult/${var.stage}/domain/authentication/service/user/infrastructure/storage/moult-user-event-store-table/arn"
  type  = "SecureString"
  value = aws_dynamodb_table.event_store_table.arn
}

resource "aws_ssm_parameter" "event_store_table_stream_arn" {
  name  ="/moult/${var.stage}/domain/user/infrastructure/storage/moult-user-event-store-table/stream/arn"
  type  = "SecureString"
