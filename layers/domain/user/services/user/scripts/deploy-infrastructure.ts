...
const initializeTerraform = async () => {
    ...
    const stateFileKey = "domain/authentication/service/user/infrastructure/terraform.tfstate";
...
const deployInfrastructure = async () => {    
    ...
    const spinner = ora(`Deploying user service infrastructure to ${chalk.black(stage)}.`)
    spinner.start();
  
    const command = `cd infrastructure && terraform apply -auto-approve --var "stage=${stage}"`;
    const success = await execAsync(command, { stdio: "pipe" });

    if (success) return spinner.succeed(`Successfully deployed user service infrastructure to ${chalk.black(stage)}.`) || true;
    else spinner.fail(`Failed to deploy user service infrastructure to ${chalk.black(stage)}.`);

};

const deployApplication = async () => {    

    const spinner = ora(`Deploying user service application to ${chalk.black(stage)}.`)
    spinner.start();
  
    const command = `sls deploy --stage=${stage}`;
    const success = await execAsync(command, { stdio: "pipe" });

    if (success) return spinner.succeed(`Successfully deployed user service application to ${chalk.black(stage)}.`) || true;
    else spinner.fail(`Failed to deploy user service application to ${chalk.black(stage)}.`);
