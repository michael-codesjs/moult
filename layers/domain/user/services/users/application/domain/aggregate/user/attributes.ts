...
export type UserAttributesSchema = CommonAttributes & {
  ...
  signature?: AttributeSchema<string>
...
export class UserAttributes extends Attributes<UserAttributesSchema> {
  constructor() {
    ...
    const entity_type = new Attribute<'USER', true>({
      required: true,
      value: 'USER',
    })
    const email = new Attribute<string>({
      required: false,
      value: null,
      validate: emailValidator,
    })
    const phone_number = new Attribute<string>({
      required: false,
      value: null,
      validate: phoneNumberValidator,
    })
    const email_verified = new Attribute<boolean>({
      required: false,
      value: null,
    })
    const phone_number_verified = new Attribute<boolean>({
      required: false,
      value: null,
    })
    const password = new Attribute<string>({ required: false, value: null })
    const name = new Attribute<string>({ required: false, value: null })
    const signature = new Attribute<string>({ required: false, value: null })
  ...
  parse(
    attributes: Partial<EntriesFromAttributesSchema<UserAttributesSchema>>,
  ): void {
