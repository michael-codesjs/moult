...
resource "aws_ssm_parameter" "event_count_table_name" {
  name  = "/moult/${var.stage}/domain/user/users/infrastructure/storage/moult-user-event-count-table/name"
  type  = "SecureString"
  value = aws_dynamodb_table.event_counts.name
}

resource "aws_ssm_parameter" "event_count_table_arn" {
  name  = "/moult/${var.stage}/domain/user/users/infrastructure/storage/moult-user-event-count-table/arn"
  type  = "SecureString"
