...
>(
  ...
  field: F
  dataSource: D
  request: RQ extends string
    ? `./adapters/primary/mapping-templates/${RQ}`
    : false
  response: RP extends string
    ? `./adapters/primary/mapping-templates/${RP}`
    : false
}
...
export const createMappingTemplate: CreateMappingTemplate = (params) => {
  ...
  return {
    ...
    request: (typeof request === 'string'
      ? `./adapters/primary/mapping-templates/${request}`
      : false) as any,
    response: (typeof response === 'string'
      ? `./adapters/primary/mapping-templates/${response}`
      : false) as any,
  }
}
