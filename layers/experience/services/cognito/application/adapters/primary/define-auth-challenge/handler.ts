...
const handler: DefineAuthChallengeTriggerHandler = async event => {
    ...
    const user_is_confirmed = event.request.userAttributes['cognito:user_status'] === 'CONFIRMED';
    ...
    if (lastChallenge.challengeName === 'SRP_A' && lastChallenge.challengeResult === true) {
    ...
    if (lastChallenge.challengeName === 'CUSTOM_CHALLENGE') {
        ...
        const failedChallenges = event.request.session
            .filter(challenge => 
