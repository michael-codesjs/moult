...
>(
  ...
  config: {
    endpoint: {
      'Fn::Sub': S extends true
        ? 'https://sync-states.${self:custom.region}.amazonaws.com/'
        : 'https://states.${self:custom.region}.amazonaws.com/'
    }
    authorizationConfig: {
  ...
  return {
    ...
    config: {
      endpoint: {
        'Fn::Sub': sync
          ? 'https://sync-states.${self:custom.region}.amazonaws.com/'
          : ('https://states.${self:custom.region}.amazonaws.com/' as any),
      },
      authorizationConfig: {
        authorizationType: 'AWS_IAM',
        awsIamConfig: {
      ...
      iamRoleStatements: [
        {
          Effect: 'Allow',
          Action: [
            sync ? 'states:StartSyncExecution' : 'states:StartExecution',
          ] as any,
