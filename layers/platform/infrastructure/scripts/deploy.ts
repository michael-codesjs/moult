...
const initializeTerraform = async () => {
    ...
    const stateFileKey = "platform/infrastructure/terraform.tfstate";
...
const deploy = async () => {    
    ...
    const spinner = ora(`Deploying platform infrastructure to ${chalk.black(stage)}.`)
    spinner.start();
  
    const command = `terraform apply -auto-approve --var "stage=${stage}"`;
    const success = await execAsync(command, { stdio: "pipe" });

    if (success) return spinner.succeed(`Successfully deployed platform infrastructure to ${chalk.black(stage)}.`) || true;
