...
import {
  ...
  GetSetMutableAttributes,
  AttributesConstructorParams,
  ToAttributeRecord,
} from './types'
import { AttributeSchema, CommonAttributes } from './types/attributes'

type CommonAttributesPlusOthers = CommonAttributes &
  Record<string, AttributeSchema<any, boolean>>

export class Attributes<
  T extends CommonAttributesPlusOthers,
> extends Publisher {
  ...
  protected defineICommon() {
    ...
    this.Attributes.creator_type = new Attribute<
      T['creator_type']['type'],
      true
    >({
      required: true,
  ...
  private defineT(params: ConstructorParameters<typeof Attributes>[0]) {
    Object.entries(params).forEach(([key, value]) => {
      if (!(value instanceof Attribute))
        throw new Error(
          "Member of attributes collection is not of type 'Attribute'.",
        )
  ...
  parse(attributes: Partial<EntriesFromAttributesSchema<T>>) {
    ...
    this.Attributes.discontinued.set(
      typeof discontinued !== 'boolean' ? false : discontinued,
      _modified,
    )
    ...
    if (creator && creator_type) {
      ...
      // if an entity's creator is not specified, it's its own creator
  ...
  set(attributes: GetSetMutableAttributes<T>) {
    this.forEachOnMutate(
      attributes as EntriesFromAttributesSchema<T>,
      (value, key, modified) => {
  ...
  collective(): EntriesFromAttributesSchema<T> {
    const collective: EntriesFromAttributesSchema<T> =
      {} as EntriesFromAttributesSchema<T> // safe to cast, will fill up below
  ...
  isPutable(): boolean {
    ...
    return !Object.values(this.Attributes).some(
      (attribute) => !attribute.isPutable(),
    )
  }
